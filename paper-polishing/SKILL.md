...
description: >-
  Polish existing academic prose into publication-quality English using
  writing-strategy principles and Academic Phrasebank phrase-level support. Use
  when the user has already-written text and wants to improve language, style,
  or readability — NOT for drafting from scratch (use paper-writing) or
  full-pipeline paper generation (use ars-suite/academic-paper). Also covers
  LaTeX layout/typesetting (排版) fixes — loose or sparse pages, stranded section
  headings, figures that don't fill the page or split across pages, "Float too
  large", multi-panel figure arrangement, and Supplementary Information that
  looks empty — via references/latex-layout.md. Triggers: polish a manuscript
  paragraph/abstract/introduction/discussion, improve writing quality, language
  editing, proofreading, Chinese academic draft → English, and Chinese phrasings
  like 论文润色、英文论文润色、语言润色、润色、改写、学术英语、英文写作、
  SCI论文润色、LaTeX排版、学术论文排版.
version: 6.1.0
author: Yuan1z skill, refactored into static/dynamic layers
---

# Academic Manuscript Polishing — Router
...
- `journal` — nature / nat-comms / generic. Default: generic. If the user names a subjournal, treat it as `nature`.
