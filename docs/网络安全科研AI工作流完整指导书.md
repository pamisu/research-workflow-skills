...
**核心定位**：本工作流基于 Claude Code Skills 体系构建，AI 承担机械性、流程性、校验性工作，核心研究判断由研究者主导，保障科研严谨性与学术合规性。Skills 已从初始 63 个精简至 15 个（16 个 SKILL.md，含 ars-suite 路由）——只保留科研通用能力，领域专项工具（威胁情报、网络流量处理、代码安全审计）已移除，由 AI 原生能力替代。
...
1. **分章节起草初稿**（`paper-writing`）
2. **分章节写作逻辑校验**（`ars-suite`）
3. **学术语言抛光**（`paper-polishing`）
4. **图表规范校验**（`academic-figure`）
5. **语言去 AI 化**（`unslop`）
6. **整体规范检查**
7. **阶段收尾**（`/neat`）
...
1. **分章节起草初稿**
   如果你面对的是实验数据、图表、笔记而非成文——先用 `paper-writing` 搭建章节框架并起草初稿。该 skill 从研究者提供的 claim/evidence/boundary 出发，按 paper_type（research/methods/hypothesis/algorithmic/review）→ section（abstract/intro/method/experiments/discussion 等）→ journal 的优先级组织论证链并输出草稿。

   指令示例（方法章节起草）：

   > 用 paper-writing 起草 Method 章节
   > 模块设计：[X, Y, Z 三个模块]，核心创新点是 [具体创新]
   > 数据集：[名称/规模/来源]，评估指标：[指标列表]
   > 实验配置：[硬件/超参/基线]
...
3. **学术语言抛光**
   已有成文章节后，调用 `paper-polishing` 对学术表达进行抛光：改善句式结构、提升术语精确度、匹配目标期刊风格。该 skill 还专门处理 LaTeX 排版问题（松散页、浮动体溢出、Supplementary Information 空白等）。

   指令示例：

   > 用 paper-polishing 抛光这段 Introduction
   > 目标期刊：IEEE S&P（IEEE 双栏格式）
   > [粘贴草稿]

   > **区分**：`paper-polishing` 用于**已有成文**的语言提升，`paper-writing` 用于**从零起草**。
   > 两者与 `ars-suite/academic-paper` 的区别：前者是轻量级单章节工具，后者是 12-agent 全流程管线（含文献搜索+引用格式化+自查），适合写整篇论文时使用。
