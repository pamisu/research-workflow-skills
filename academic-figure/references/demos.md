...
materials for the Python track only. Keep the normal `academic-figure` contract first:
...
- Grouped bars: start with `figure_ImmunoStruct`, `figure_CellSpliceNet`, or
  `figure_brainteaser`; then apply the tighter publication export and font rules in
  `api.md`.
...
guidance is folded into `academic-figure`:
