...
`academic-figure` 用于生成可投稿级科研图，面向顶级期刊和会议，同时支持 Python 与 R 两条绘图路径。
...
该技能参考了 [figures4papers](https://github.com/ChenLiu-1996/figures4papers) 中来自顶级机器学习和生物信息学论文的生产脚本。原始 demo 脚本和预览图也打包在 `assets/figures4papers/` 中，供模式级改写使用。
...
下面的图库按 chart family 分类。每张预览都是紧凑的 4 x 4 小面板 atlas，用来展示可组合进学术发表质量结果图的视觉语法范围。
...
推荐在密集多面板图中使用统一低饱和配色：一个 coherent baseline family 加一个 coherent hero family，绿色/红色只用于差值标记或真正有方向性的语义。
