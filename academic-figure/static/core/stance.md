...
Prefer **unified method families across all panels** over maximal hue separation. For dense dense multi-panel journal figure pages, use the low-saturation `low-saturation pastel` family described in `references/api.md` and reserve green/red mainly for gains, drops, and other directional cues.
...
- When the user asks for publication-quality style rather than ML/NMI-specific style, read `references/journal-figure-archetypes.md` before choosing layout.
