# `targeted-citation` 技能

`targeted-citation` 用于把论文段落、手稿片段或单条科研判断转化为可审查的定向引用候选（默认限定在 CS/安全顶会顶刊范围内），并导出可被文献管理器直接导入的文件。

这个技能支持中文输入。用户可以直接提出"分段引用""安全会议引用""顶会引用""CS 顶会""补引用""支撑文献""导出 Zotero"等需求；技能会用英文科学概念检索文献，同时默认返回中文审查说明。

## 功能

- 将手稿拆分为可引用的 claim 单元，并生成稳定编号，例如 `S001`、`S002`、`S003`。
- 为每个片段生成面向 Crossref、DBLP、IEEE Xplore、ACM DL 等结构化元数据源的检索式。
- 将结果限制在 IEEE S&P、CCS、NDSS、USENIX Security 等安全顶会，或扩展到更广的 CS 会议范围。
...
- DBLP、Crossref、IEEE Xplore、ACM DL 结构化元数据与 DOI 记录。
- IEEE S&P、ACM CCS、NDSS、USENIX Security 等官方 proceedings 页面。
- 二级学术索引（Google Scholar、Semantic Scholar）只作为发现线索，不能作为唯一支撑依据。

## 文件结构

该技能采用 router/static-dynamic 结构：`SKILL.md` 负责短路由，`manifest.yaml` 决定常驻内容和按需参考文件。`targeted-citation` 是线性工作流，没有内容轴，因此主要由 core 常驻内容和按需 references 组成。
...
├── static/
│   └── core/                    # 始终加载
│       ├── principles.md        # 产物、会议范围、来源层级、检索规则
│       ├── chinese-mode.md      # 中文用户模式
│       └── workflow.md          # 7 步工作流与报告格式
├── references/                  # 按需打开
│   ├── script-usage.md          # targeted_citation.py 参数与长文批处理
│   ├── journal-scope.md         # CS/安全会议层级与范围定义
│   ├── ris-endnote.md           # ENW/RIS/Zotero RDF 导出说明
│   └── search-strategy.md       # claim 拆解、支撑等级和检索策略
└── scripts/
    └── targeted_citation.py     # 本地 CLI
```
...
- 将长文本拆成"片段-候选文献"对应表。
- 限定只检索 `security-top4`（S&P/CCS/NDSS/USENIX）、`security-all` 或 `custom` 范围。
...
- `journal-scope.md`：CS/安全会议层级与范围定义。
- `ris-endnote.md`：`ENW`、`RIS`、Zotero `RDF` 导出说明。
- `scripts/targeted_citation.py`：本地 CLI，用于分段、检索、导出和 HTML 审查页生成。

## 常用 CLI 参数

- `--batch-size 2`：将长文本拆成更小批次处理。
- `--max-segments 12`：限制一次运行处理的片段数。
- `--max-retries 2`：重试临时检索失败。
- `--sleep 0.3`：缩短请求间默认等待。
