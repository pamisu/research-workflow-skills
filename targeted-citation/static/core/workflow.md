...
Prefer `scripts/targeted_citation.py` when internet access is available. The full flag list, polite-pool/rate-limit options, and the long-article batch strategy are in `references/script-usage.md`. A minimal run:

```bash
python scripts/targeted_citation.py \
  --text "PASTE MANUSCRIPT TEXT HERE" \
  --scope security-top4 \
  --outdir /tmp/targeted-citation \
  --format enw \
  --with-artifacts
```
...
检索范围
- [security-top4 / security-all / cs-conferences / custom, plus date limits]
...
Put the HTML browser path FIRST, above everything else, so the user can immediately open and browse candidates. If no suitable target-venue paper exists, say so plainly and suggest the best nearby options from broader literature only if the user wants wider coverage. If the text is long, mention the batch strategy used, especially when you limited the run with `--batch-size` or `--max-segments`.
