#!/usr/bin/env python3
"""
Segment manuscript text, search targeted citation candidates from specified CS/security venues,
and export an EndNote file. By default the script writes only one output file in `.enw` format.
...
USER_AGENT = "codex-targeted-citation/1.0 (mailto:unknown@example.com)"
...
def crossref_headers(mailto: str | None = None) -> dict[str, str]:
    return {"User-Agent": USER_AGENT if not mailto else f"codex-targeted-citation/1.0 (mailto:{mailto})"}
