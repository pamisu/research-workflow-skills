...
Open this reference when running `scripts/targeted_citation.py` or when the input is long enough to need batching. It expands step 3 of the workflow.

## Running the script

Start with `scripts/targeted_citation.py` when internet access is available:
...
- `--scope security-top4`: IEEE S&P, ACM CCS, NDSS, USENIX Security only.
- `--scope security-all`: Top 4 plus ACSAC, RAID, ESORICS, PETS, CSF, AsiaCCS.
- `--scope cs-conferences`: Broader CS venues including AAAI, IJCAI, NeurIPS.
- `--scope custom --venues venues.txt`: User-provided venue list.
