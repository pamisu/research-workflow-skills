# `reviewer-response` 技能

`reviewer-response` 用于起草、审查和修改逐点回复审稿人的 response letter，适用于各类期刊和会议等学术场景的返修。
...
- 准备各类期刊和会议返修（如 ACM CCS、IEEE S&P、NDSS、USENIX Security 等）。
- 回复大修或小修意见。
- 将审稿意见转化为手稿修改清单。
...
该技能采用 router/static-dynamic 结构：`SKILL.md` 负责短路由，`manifest.yaml` 加载常驻 core 和按需 references。`reviewer-response` 是线性工作流，没有内容轴。
